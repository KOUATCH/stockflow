...
// import { useNotification } from "@/hooks/session-pos-sync/use-notification"
...
export function POSTerminalFinal({ organizationId, locationId, terminalId, userId }: pOSStationProps): ReactElement {
  console.log(organizationId, locationId, terminalId, userId)
  ...
  //   try {
  //     notification.info("Processing Payment", "Please wait while we process your transaction...")
  //     notification.info("Verification", `This is the user ID in process payment ${userId} of ${User.displayName}`)
  ...
  //       notification.error("Sale Failed", saleResult?.error || "Failed to create sale")
  //       throw new Error(saleResult?.error || "Sale creation failed - no sale ID returned")
  //     }
  ...
  //         amount: calculateTotal(),
  //         method: paymentMethod, // Use the enum value directly
  ...
  //         organizationId, // Add organizationId to each update object
  //       }))
  ...
  //         notification.warning("Inventory Warning", "Sale completed but inventory may not be updated")
  //         notification.info("Verification", `This is the user ID in process payment ${userId} of ${User.displayName}`)
  ...
  //         referenceType: "salesOrder", // or the appropriate string for your system
  //         createdById: userId,
  //         serialNumbers: [], // or provide actual serial numbers if available
  //         userId: userId // keep if needed elsewhere, but not required by InventoryTransaction
  //       }))
  ...
  //         // Don't fail for transaction logging issues
  //         notification.warning("Transaction Log Warning", "Sale completed but transaction log may be incomplete")
  //       }
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      // FIXED: Improved error checking with detailed logging
      console.log("[FRONTEND] Sale result:", saleResult)

      if (!saleResult?.success) {
        const errorMessage = saleResult?.error || "Failed to create sale - unknown error"
        console.error("[FRONTEND] Sale creation failed:", errorMessage)
        notification.error("Sale Failed", errorMessage)
        throw new Error(errorMessage)
      }

      if (!saleResult?.data?.id && !saleResult?.saleId) {
        console.error("[FRONTEND] Sale creation succeeded but no ID returned:", saleResult)
        notification.error("Sale Failed", "Sale creation failed - no sale ID returned")
        throw new Error("Sale creation failed - no sale ID returned")
      }

      const saleId = saleResult.data?.id || saleResult.saleId
      console.log("[FRONTEND] Sale created successfully with ID:", saleId)

      clearInterval(progressInterval)
      setPaymentProgress(100)

      // FIXED: Updated payment creation to match the corrected createPayment function signature
      const paymentData = {
        salesOrderId: saleId,
        amount: calculateTotal(),
        method: paymentMethod as PaymentMethod,
        processedById: userId, // This will be handled in the server action
      }

      console.log("[FRONTEND] Creating payment:", paymentData)
      await createPaymentMutation.mutateAsync(paymentData)

      // Update inventory - this is handled in the main createSale transaction now
      // But we can still call it for additional validation/logging
      const inventoryUpdates = cart.map((item) => ({
        itemId: item.itemId,
        locationId: selectedLocationId,
        quantityChange: item.quantity, // Positive number, the backend will handle the negation
        organizationId,
      }))
      ...
      try {
        ...
        console.error("Inventory update failed:", inventoryError)
        // Don't fail the entire transaction for inventory issues since it's handled in main transaction
        notification.warning("Inventory Warning", "Sale completed but inventory may need manual verification")
      }

      // Create inventory transactions for logging (optional since it's in main transaction)
      const transactions = cart.map((item) => ({
        itemId: item.itemId,
        locationId: selectedLocationId,
        organizationId,
      ...
      try {
        ...
        console.error("Transaction creation failed:", transactionError)
        // Don't fail for transaction logging issues since it's handled in main transaction
        notification.warning("Transaction Log Warning", "Sale completed but transaction log may need verification")
      }
      ...
      console.error("[FRONTEND] Payment processing failed:", error)
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred"
      notification.error("Payment Failed", errorMessage, {
        duration: 10000,
      })
    } finally {
      setIsProcessing(false)
  ...
  const updateQuantity = (cartItemId: string, quantity: number) => {
    ...
    if (item && quantity > availableStock && availableStock > 0) {
      notification.warning("Stock Limit Reached", `Only ${availableStock} of ${item.name} available in stock.`)
      notification.warning("User Id passed", `${userId} `)
      setCart(
        cart.map((item) =>
          item.id === cartItemId
            ? {
      ...
      notification.error("Out of Stock", `${item.name} is out of stock.`)
      return
    }
