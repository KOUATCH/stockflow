...
export function CashDrawerManager({ terminalId, userId, locationId, organizationId }: CashDrawerManagerProps) {
  ...
  const [recentEvents, setRecentEvents] = useState<cashDrawerTransaction[]>([
    {
      id: "event-1",
      type: "SALE" as cashDrawerTransactionType,
      amount: 25.5,
      reason: "Sale transaction",
      cashDrawerId: "drawer-1",
      sessionId: "session-1",
      userId,
      balanceBefore: 1025.25,
      balanceAfter: 1050.75,
      createdAt: new Date(Date.now() - 300000), // 5 minutes ago
    },
    {
      id: "event-2",
      type: "CASH_IN" as cashDrawerTransactionType,
      amount: 100.0,
      reason: "Change fund addition",
      notes: "Added change for busy period",
      cashDrawerId: "drawer-1",
      sessionId: "session-1",
      userId,
      balanceBefore: 925.25,
      balanceAfter: 1025.25,
      createdAt: new Date(Date.now() - 900000), // 15 minutes ago
    },
    {
      id: "event-3",
      type: "CASH_OUT" as cashDrawerTransactionType,
      amount: 50.0,
      reason: "Bank deposit",
      notes: "Deposited excess cash",
      cashDrawerId: "drawer-1",
      sessionId: "session-1",
      userId,
      balanceBefore: 975.25,
      balanceAfter: 925.25,
  ...
  const variance = cashDrawerStatus.currentBalance - expectedBalance
  ...
  const handleCashTransaction = async (amount: number, reason: string, notes?: string) => {
    ...
    try {
      ...
      const transactionEvent: cashDrawerTransaction = {
        id: `event-${Date.now()}`,
        type: transactionType as cashDrawerTransactionType,
        amount,
        reason,
        notes,
        cashDrawerId: cashDrawerStatus.id,
