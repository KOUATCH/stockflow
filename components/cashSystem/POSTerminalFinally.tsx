...
export function POSTerminalFinally({ organizationId, locationId, terminalId, userId }: pOSStationProps): ReactElement {
  ...
  const CartItemDisplay = ({ item }: { item: CartItem }) => (
    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-white to-gray-50 rounded-xl border border-gray-200 hover:shadow-md transition-all group">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          <div className="font-bold text-lg text-emerald-600">${item.lineTotal.toFixed(2)}</div>
          {item.taxAmount > 0 && <div className="text-xs text-gray-500">+${item.taxAmount.toFixed(2)} tax</div>}
        </div>
...
export default POSTerminalFinally
