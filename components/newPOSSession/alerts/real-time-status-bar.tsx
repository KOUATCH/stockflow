...
export function RealTimeStatusBar({
  ...
  const { lowStockCount, criticalStockCount } = useLowStockMonitoring(locationId, organizationId)
  const { systemMetrics, systemHealth } = useSystemMonitoring(organizationId, locationId)
  const { currentSession, sessionLoading } = useSessionManagement(terminalId ?? "")
    ...
    >
      <CardContent className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-8 gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <Package
              className={`h-4 w-4 ${criticalStockCount > 0 ? "text-red-600" : lowStockCount > 0 ? "text-orange-600" : "text-green-600"
                }`}
            />
            <div>
              <div
              ...
              <div className="text-xs text-muted-foreground">{criticalStockCount > 0 ? "Critical" : "Low Stock"}</div>
            </div>
          </div>
