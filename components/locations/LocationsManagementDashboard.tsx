...
export default function LocationsManagementDashboard({
  ...
  const columns = useMemo<ColumnDef<LocationManagementRow>[]>(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="grid min-w-[13rem] grid-cols-2 gap-2">
            ...
            <CountPill icon={Package} label={t.inventoryTx} value={location.inventoryTransactionsCount} locale={locale} />
          </div>
        )
      },
    },
    ...
    {
      id: "activity",
      accessorFn: (row) => row.salesOrdersCount + row.purchaseOrdersCount + row.stockTransfersInCount + row.stockTransfersOutCount,
      header: ({ column }) => (
        <HeaderButton onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>
      ...
      <section className="dashboard-glass-panel overflow-hidden rounded-lg text-[var(--dash-text)]">
        <div className="border-b border-[var(--dash-border-subtle)] bg-[rgba(12,20,24,0.58)] px-5 py-4 sm:px-6">
          <div className="flex min-w-0 flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:min-w-[36rem]">
              <SummaryTile icon={MapPin} label={t.totalLocations} value={locations.length} locale={locale} tone="brand" />
              <SummaryTile icon={CheckCircle2} label={t.activeLocations} value={summary.activeLocations} locale={locale} tone="success" />
              <SummaryTile icon={Package} label={t.stockValue} value={formatCurrency(summary.stockValue, locale)} tone="gold" />
              <SummaryTile icon={Store} label={t.activeSessions} value={summary.activeSessions} locale={locale} tone="spruce" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5 p-5 sm:p-6">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <MetricPanel icon={Warehouse} label={t.defaultLocation} value={summary.defaultLocation || t.noDefault} tone="brand" />
            <MetricPanel icon={ShieldAlert} label={t.approvalLocations} value={formatNumber(summary.approvalLocations, locale)} tone="gold" />
            <MetricPanel icon={Terminal} label={t.posTerminals} value={formatNumber(summary.posTerminals, locale)} tone="spruce" />
            <MetricPanel icon={WalletCards} label={t.cashDrawers} value={formatNumber(summary.cashDrawers, locale)} tone="success" />
          </div>

          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
            <div className="rounded-lg border border-[var(--dash-border-subtle)] bg-[rgba(37,57,67,0.34)] p-4">
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_10rem_10rem_12rem]">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <Select value={density} onValueChange={(value) => setDensity(value as Density)}>
                  <SelectTrigger className="dashboard-control h-10 w-40 rounded-lg">
              ...
              {selectedRows.length ? (
                <div className="mt-3 rounded-lg border border-[var(--dash-brand)] bg-[var(--dash-brand-soft)] px-3 py-2 text-sm text-[var(--dash-brand-strong)]">
...
function MetricPanel({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-[var(--dash-border-subtle)] bg-[rgba(37,57,67,0.38)] p-4">
      <span className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border", toneClass(tone))}>
        <Icon className="h-5 w-5" />
      </span>
      <div className="min-w-0">
        <p className="truncate text-xs text-[var(--dash-text-faint)]">{label}</p>
        <p className="mt-1 truncate text-base font-semibold text-[var(--dash-text)]">{value}</p>
      </div>
    </div>
  )
}
