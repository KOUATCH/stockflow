...
import { PRESENCE_STATUS_COLORS, PresenceStatusResponse } from '@/types/presence';
...
export function PresenceStatusCard({
  ...
  const getStatusMessage = () => {
    ...
    switch (status.status) {
      ...
      case 'ON_BREAK':
        return `You're currently on a ${session.currentBreak?.breakType?.toLowerCase() || 'break'}. Duration: ${session.currentBreak ? formatDuration(
      ...
      default:
        return `Current status: ${status.status.replace('_', ' ').toLowerCase()}`;
    }
  };
  ...
  return (
    <Card className={cn("w-full", className)}>
      <CardContent className="p-6">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <Badge className={PRESENCE_STATUS_COLORS[status.status]}>
                  {status.status === 'CLOCKED_OUT' ? 'OFFLINE' : status.status?.replace('_', ' ')}
              ...
              {session && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">
                      {formatDuration(session.totalBreakMinutes)}
                    </p>
                  </div>
                </div>
              )}
        ...
        {isOnBreak && session?.currentBreak?.expectedDuration && (
          (() => {
              ...
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-sm text-yellow-800">
                    <span className="font-medium">Extended Break:</span> Your break has exceeded the expected duration by{' '}
                    {formatDuration(currentBreakDuration - session.currentBreak.expectedDuration)} minutes.
                  </p>
                </div>
              </div>
            ) : null;
...
// You may want to move these to a separate utils file
