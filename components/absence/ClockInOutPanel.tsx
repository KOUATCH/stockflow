...
export function ClockInOutPanel({ currentStatus, organizationId }: ClockInOutPanelProps) {
  ...
  } = useOrgLocationsNew(organizationId, { enabled: !!organizationId })
  ...
  console.log("Clock-in Debug:", {
    ...
    locationsLoading,
    locationsError,
    locationsResponseSuccess: locationsResponse?.success,
    locationsResponseData: locationsResponse?.data,
    locationsResponseError: locationsResponse?.error,
  })
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isWorking ? "Manage your current work session" : "Start your work session by clocking in"}
          </CardDescription>
        </CardHeader>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <Badge
                      className={`${currentStatus.status === "CLOCKED_IN"
                    ...
                    {currentStatus.session && (
                      <span className="text-sm text-gray-600">at {currentStatus.session.location.name}</span>
                    )}
                  </div>
                </div>
                {currentStatus.session && (
                  <div className="text-right">
                    ...
                    <p className="font-medium">{new Date(currentStatus.session.clockInTime).toLocaleTimeString()}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedLocationId} onValueChange={setSelectedLocationId} disabled={locationsLoading}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={
                          locationsLoading
                  ...
                  {locationsError && (
                    <Alert>
                      ...
                      <AlertDescription>Error loading locations: {locationsError.message}</AlertDescription>
                    </Alert>
                  )}
                  ...
                  {!locationsLoading && !locationsError && locations.length === 0 && (
                    <Alert>
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription>
                        No locations found for this organization (ID: {organizationId}).
                        {locationsResponse?.success === false &&
                  ...
                  {process.env.NODE_ENV === "development" && (
                    <div className="text-xs text-gray-500 mt-2">
                      ...
                      <div>Loading: {locationsLoading ? "Yes" : "No"}</div>
                      <div>Response Success: {locationsResponse?.success ? "Yes" : "No"}</div>
                      <div>Locations Count: {locations.length}</div>
                      {locationsResponse?.error && <div>Response Error: {locationsResponse.error}</div>}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-4 border-t pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setBreakDuration(Number.parseInt(value))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex space-x-3">
                <Button
                  ...
                  disabled={loading}
                  variant="outline"
                  className="flex-1 border-yellow-600 text-yellow-600 hover:bg-yellow-50 bg-transparent"
                >
          ...
          {isOnBreak && (
            <div className="space-y-4 border-t pt-4">
              <Alert>
                ...
                <AlertDescription>You are currently on a break. Click "End Break" to resume work.</AlertDescription>
              </Alert>
