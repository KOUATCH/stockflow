...
// Simplified validation schema - matches server schema exactly
const itemCreationSchema = z.object({
  organizationId: z.string().min(1, "Organization ID is required"),
  // Core required fields - explicitly handle null/undefined
  nameEn: z.preprocess(
    (val) => val === null || val === undefined ? "" : String(val),
    z.string().min(1, "English name is required").max(255)
  ),
  nameFr: z.string().optional().nullable(),
  sku: z.preprocess(
    (val) => val === null || val === undefined ? "" : String(val),
    z.string().min(1, "SKU is required").max(128)
  ),
...
const FORM_STEPS = [
  ...
  { id: 'details', title: 'Details', icon: Hash, description: 'SKU and product specs' },
...
export function ModernCreateItemForm({
  ...
  const form = useForm<ItemCreationFormData>({
    ...
    defaultValues: {
      ...
      nameFr: initialData?.nameFr || null,
      descriptionEn: initialData?.descriptionEn || null,
      descriptionFr: initialData?.descriptionFr || null,
      sku: initialData?.sku || "",
      ...
      maxStockLevel: initialData?.maxStockLevel || null,
  ...
  useEffect(() => {
    // Initialize required fields with empty strings if they're undefined/null
    const values = form.getValues();
    const updates: Partial<ItemCreationFormData> = {};

    if (values.nameEn === undefined || values.nameEn === null) {
      updates.nameEn = "";
    }
    if (values.sku === undefined || values.sku === null) {
      updates.sku = "";
    }
    if (values.organizationId === undefined || values.organizationId === null) {
      updates.organizationId = organizationId;
    }
    if (values.imageUrls === undefined || values.imageUrls === null) {
      updates.imageUrls = DEFAULT_IMAGE_URL;
    }

    if (Object.keys(updates).length > 0) {
      console.log("Initializing form fields:", updates);
      Object.entries(updates).forEach(([key, value]) => {
        form.setValue(key as keyof ItemCreationFormData, value, { shouldValidate: false });
      });
      // Clear any existing errors after initialization
  ...
  // Form validation by step with detailed error checking
  const validateStep = async (step: FormStep, silent = false): Promise<boolean> => {
    ...
    // Special handling for media step - check if image is uploaded
    if (step === 'media' && (!itemImageUrl || itemImageUrl === DEFAULT_IMAGE_URL)) {
      form.setValue('imageUrls', DEFAULT_IMAGE_URL)
      if (!silent) {
        warning("Image Required", "Please upload a product image to continue")
      }
      return false
    }

    // Update form imageUrls field with current image URL
    if (itemImageUrl && itemImageUrl !== DEFAULT_IMAGE_URL) {
      form.setValue('imageUrls', itemImageUrl, { shouldValidate: true })
    }
    ...
    if (!result && !silent) {
      ...
      fieldsToValidate.forEach(field => {
        ...
        if (fieldError?.message) {
          errors.push(fieldError.message as string)
        }
      })
  ...
  const handleSubmit = async (data: ItemCreationFormData) => {
    ...
    // Debug form field values before validation
    const debugNameValue = form.getValues("nameEn");
    const debugSkuValue = form.getValues("sku");
    console.log("Name field value:", debugNameValue, "Type:", typeof debugNameValue);
    console.log("SKU field value:", debugSkuValue, "Type:", typeof debugSkuValue);

    // Perform complete form validation first
    const isValid = await form.trigger();
    console.log("Complete form validation result:", isValid);

    if (!isValid) {
      error("Validation Failed", "Please check all required fields and correct any errors.");
      console.log("Current form errors:", form.formState.errors);

      // Ensure required string fields are not null/undefined
      const currentValues = form.getValues();
      if (currentValues.nameEn === null || currentValues.nameEn === undefined) {
        form.setValue("nameEn", "", { shouldValidate: true });
      }
      if (currentValues.sku === null || currentValues.sku === undefined) {
        form.setValue("sku", "", { shouldValidate: true });
      }
      if (currentValues.organizationId === null || currentValues.organizationId === undefined) {
    ...
    const nameValue = String(currentValues.nameEn || "").trim();
    const skuValue = String(currentValues.sku || "").trim();

    if (!nameValue || nameValue.length === 0) {
      error("Validation Failed", "Product name is required. Please fill out the Basic Information step.");
      setCurrentStep('basic');
      return;
    }

    if (!skuValue || skuValue.length === 0) {
      // Try to auto-generate SKU if missing and we have a name
      if (nameValue && nameValue.length > 0) {
        ...
        info("Auto-Generated SKU", `SKU automatically created: ${autoSku}`);
        // Update current values with the new SKU
        currentValues.sku = autoSku;
      } else {
        error("Validation Failed", "SKU is required. Please fill out the Product Details step.");
        setCurrentStep('details');
        return;
      }
    }

    if (currentValues.costPrice === undefined || currentValues.costPrice === null || currentValues.costPrice < 0) {
      error("Validation Failed", "Valid cost price is required. Please fill out the Pricing step.");
      setCurrentStep('pricing');
      return;
    }

    if (currentValues.sellingPrice === undefined || currentValues.sellingPrice === null || currentValues.sellingPrice < 0) {
      error("Validation Failed", "Valid selling price is required. Please fill out the Pricing step.");
      setCurrentStep('pricing');
      return;
    }

    if (!currentValues.organizationId) {
      error("Validation Failed", "Organization ID is missing. Please refresh the page.");
      return;
    }
    ...
    // Update imageUrls field with current image URL before submission
    if (itemImageUrl && itemImageUrl !== DEFAULT_IMAGE_URL) {
      form.setValue('imageUrls', itemImageUrl, { shouldValidate: true })
      // Update current values to include the image URL
      currentValues.imageUrls = itemImageUrl
      currentValues.thumbnail = itemImageUrl
    }

    const operationId = operationStart("Creating Product")

    try {
      // Use current form values and prepare data for submission
      const submitData = {
        ...currentValues,
        descriptionEn: currentValues.descriptionEn || null,
        descriptionFr: currentValues.descriptionFr || null,
        categoryId: currentValues.categoryId || null,
        brandId: currentValues.brandId || null,
        unitId: currentValues.unitId || null,
        taxRateId: currentValues.taxRateId || null,
        dimensions: currentValues.dimensions || null,
        maxStockLevel: currentValues.maxStockLevel || null,
      ...
      if (action) {
        // Server action approach - this will redirect on success or throw on error
        ...
        // Show success notification before calling action (since it redirects on success)
        operationComplete("Product Created", `${submitData.nameEn} has been successfully added to your inventory with SKU: ${submitData.sku}`)
        ...
        operationComplete("Product Created", `${data.nameEn} has been successfully added to your inventory with SKU: ${data.sku}`)
      }
    } catch (err) {
      console.error("Failed to create item:", err)
      operationComplete("Creation Failed", err instanceof Error ? err.message : "Failed to create product. Please check your information and try again.")
      error("Creation Failed", err instanceof Error ? err.message : "Failed to create product. Please check your information and try again.")
    }
  }
  ...
  const avatarFallback = nameEn
    ? nameEn.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
    : "??"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-sm text-muted-foreground">
                      Optional localized name for French users
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base font-semibold text-foreground">English Product Description</FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base font-semibold text-foreground">French Product Description</FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder="Describe this product in French..."
                        className="min-h-[120px] bg-background border-2 border-border focus:border-emerald-500 rounded-xl shadow-sm resize-none"
                        {...field}
                      />
                    </FormControl>
                    <FormDescription className="text-sm text-muted-foreground">
                      Optional localized description for French users
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
        ...
        return (
          <div className="space-y-6">
            ...
            {validationErrors.length > 0 && (
              <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 mb-6">
                ...
                <ImageUploadButtonModernOriginal
                  ...
                  setImageUrl={(url: string) => {
                    ...
                    if (url && url !== DEFAULT_IMAGE_URL) {
                      ...
                      setValidationErrors([])
                      // Clear any imageUrls errors specifically
                        ...
                        {Object.keys(form.formState.errors).length > 0 && (
                          <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700">
                            <div className="flex items-start gap-3">
                              <AlertTriangle className="h-5 w-5 text-red-600 dark:text-red-400 mt-0.5 flex-shrink-0" />
                              <div>
                                <h4 className="font-semibold text-red-900 dark:text-red-100 mb-2">Please fix these errors to create the product:</h4>
                                <ul className="text-sm text-red-800 dark:text-red-200 space-y-1 list-disc list-inside">
                                  {Object.entries(form.formState.errors).map(([field, error], index) => (
                                    <li key={index}><strong>{field}:</strong> {error?.message as string}</li>
                                  ))}
                                </ul>
                              </div>
                            </div>
                        ...
                        {currentStepIndex === FORM_STEPS.length - 1 ? (
                          <Button
                            ...
                            disabled={
                              ...
                              isLoading ||
                              isImageUploading ||
                              (!isEditMode && (!itemImageUrl || itemImageUrl === DEFAULT_IMAGE_URL)) ||
                              Object.keys(form.formState.errors).length > 0
                            }
                            className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            onClick={async () => {
                              // Update imageUrls field before validation
                              if (itemImageUrl && itemImageUrl !== DEFAULT_IMAGE_URL) {
                                form.setValue('imageUrls', itemImageUrl, { shouldValidate: true })
                              }
                              ...
                              console.log("Required field check:", {
                                ...
                                nameLength: values.nameEn?.length,
                                sku: values.sku,
                                skuLength: values.sku?.length,
                          ...
                          >
                            ...
                            ) : (!isEditMode && (itemImageUrl === DEFAULT_IMAGE_URL || !itemImageUrl)) ? (
                              <>
