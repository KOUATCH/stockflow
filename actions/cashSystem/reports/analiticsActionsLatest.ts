...
export async function getSalesAnalytics(
  ...
  try {
    ...
    // Get sales data from mock database
    ...
    const saleItems = db.saleItems.filter((item) => sales.some((sale) => sale.id === item.saleId))
...
export async function getCashReconciliationReports(
  ...
  try {
    ...
    return sessions.map((session) => {
      ...
      const cashTransactions = db.cashDrawerTransactions.filter((t) => t.sessionId === session.id)
