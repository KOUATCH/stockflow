...
import type { Customer, PaymentMethod, PaymentStatus, SalesOrderStatus } from "@prisma/client";
...
// ===== UTILITY FUNCTIONS =====
function calculateLineTotal(unitPrice: number, quantity: number, discountAmount = 0, taxAmount = 0): number {
...
export async function createSalesOrder(
  ...
  try {
    ...
    const receiptNumber = generateUniqueNumber("RCP")
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      // Process payments
      const createdPayments = await processPayments(tx, salesOrder.id, saleData, changeAmount)
    ...
    const sale: SaleResponse = {
      ...
      taxRate: result.salesOrder.taxAmount,
...
async function processSaleItems(tx: any, salesOrderId: string, saleData: CreateSaleData) {
  const createdLines = []

  for (const item of saleData.lines) {
    const lineTotal = calculateLineTotal(item.unitPrice, item.quantity, item.discount, item.taxAmount)

    const taxRate = item.unitPrice > 0 ? item.taxAmount / (item.unitPrice * item.quantity) : 0
...
async function processPayments(tx: any, salesOrderId: string, saleData: CreateSaleData, changeAmount: number) {
...
async function updateSessionTotals(tx: any, saleData: CreateSaleData, paymentData: CreateSaleData["payments"][0]) {
...
// ===== QUERY FUNCTIONS =====
export async function getSessionSales(sessionId: string, limit = 50): Promise<SaleResponse[]> {
  try {
    if (!sessionId) {
...
export async function getSaleById(saleId: string): Promise<SaleResponse | null> {
  try {
    ...
    return salesOrder ? mapSalesOrderToResponse(salesOrder) : null
  } catch (error) {
...
export async function voidSale(
  ...
  try {
    if (!saleId?.trim() || !userId?.trim() || !reason?.trim()) {
    ...
    await db.$transaction(async (tx) => {
      ...
      // Restore inventory
      await restoreInventoryLevels(tx, salesOrder, userId)

      // Reverse cash transactions
      await reverseCashTransactions(tx, salesOrder, userId, reason)
...
// ===== HELPER FUNCTIONS FOR VOID =====
async function restoreInventoryLevels(tx: any, salesOrder: any, userId: string) {
  for (const line of salesOrder.lines) {
    ...
    if (inventoryLevel) {
      await tx.inventoryLevel.update({
        where: { id: inventoryLevel.id },
        data: {
          quantityOnHand: inventoryLevel.quantityOnHand + line.quantity,
          quantityAvailable: inventoryLevel.quantityAvailable + line.quantity,
          lastTransactionAt: new Date(),
        },
      })
      ...
      await tx.inventoryTransaction.create({
        data: {
          ...
          notes: "Inventory restored due to sale void",
          balanceAfter: inventoryLevel.quantityOnHand + line.quantity,
        },
      })
    }
  }
...
async function reverseCashTransactions(tx: any, salesOrder: any, userId: string, reason: string) {
  const cashPayments = salesOrder.payments.filter((p: any) => p.method === "CASH")

  if (cashPayments.length > 0 && salesOrder.sessionId) {
    const session = await tx.pOSSession.findUnique({
      where: { id: salesOrder.sessionId },
      include: {
        terminal: {
          include: {
            CashDrawer: true,
    ...
    if (session?.terminal.CashDrawer?.[0]) {
      const cashDrawer = session.terminal.CashDrawer[0]
      const totalCashAmount = cashPayments.reduce((sum: number, p: any) => sum + p.amount, 0)
      const totalChangeGiven = cashPayments.reduce((sum: number, p: any) => sum + (p.changeGiven || 0), 0)
      const netCashToReverse = totalCashAmount - totalChangeGiven
      const newBalance = cashDrawer.currentBalance - netCashToReverse
      ...
      await tx.cashDrawerTransaction.create({
        data: {
          ...
          userId,
          type: "CASH_OUT",
          amount: netCashToReverse,
          reason: `Void sale ${salesOrder.orderNumber}`,
          notes: reason,
          balanceBefore: cashDrawer.currentBalance,
          balanceAfter: newBalance,
        },
      ...
      await tx.pOSSession.update({
        where: { id: salesOrder.sessionId },
        data: {
          totalSales: { decrement: salesOrder.total },
          cashTotal: { decrement: netCashToReverse },
