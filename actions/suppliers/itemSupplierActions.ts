...
export async function getItemSupplierById(id: string) {
  try {
    ...
    const itemSupplier = await db.itemSupplier.findFirst({
      where: {
        id: id,
        ...itemSupplierOrgWhere(session.user.organizationId)
      },
      include: {
        item: {
          select: {
...
export async function createItemSupplier(data: ItemSupplierData) {
  try {
    ...
    const [item, supplier] = await Promise.all([
      db.item.findFirst({
        where: { id: data.itemId, organizationId: session.user.organizationId },
        select: { id: true },
      }),
      db.supplier.findFirst({
        where: { id: data.supplierId, organizationId: session.user.organizationId },
        select: { id: true },
      }),
    ])
