  ...
  try {
    ...
    for (const line of data.lines) {
      ...
      if (!inventory || inventory.quantity < line.quantity) {
        ...
        throw new Error(
          `Insufficient inventory for ${item?.name}. Available: ${inventory?.quantity || 0}, Required: ${line.quantity}`,
        )
      }
    }
...
export async function reserveInventory(
  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const totalReserved = currentReservations._sum.reservedQuantity || 0
