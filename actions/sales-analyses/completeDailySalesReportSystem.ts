...
export const dailySalesQueryKeys = {
  ...
  locations: (organizationId: string) =>
    [...dailySalesQueryKeys.all, 'locations', organizationId] as const,
}
    ...
    onSuccess: (data, variables) => {
      if (data.success) {
        ...
        queryClient.invalidateQueries({
          queryKey: dailySalesQueryKeys.report(
            variables.date, 
...
export function useDailySalesReportHistory(
  organizationId: string,
  locationId?: string,
  limit: number = 30
) {
...
export function useDailySalesReporting(
  date: string,
  locationId: string,
  organizationId: string
) {
