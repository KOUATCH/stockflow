...
class MockDatabase {
  ...
  item = {
    findMany: async ({
      ...
      return paginatedItems.map((item) => {
        ...
        if (include?.inventoryLevels) {
          let inventoryLevels = this.inventoryLevels.filter((il) => il.itemId === item.id)
          if (
            include?.inventoryLevels &&
            typeof include.inventoryLevels === "object" &&
            include.inventoryLevels.where?.locationId
          ) {
  ...
  posSession = {
    findFirst: async ({
      ...
      if (include?.user) {
        // Mock user data since we don't have users in this simplified version
        result.user = {
          id: session.userId,
          firstName: "Demo",
          lastName: "User",
  ...
  salesOrder = {
    findMany: async ({
      ...
      where?: { sessionId?: string; locationId?: string; organizationId?: string }
      include?: { lines?: boolean; customer?: boolean; payments?: boolean }
    } = {}) => {
      let filtered = this.salesOrders
      if (where?.sessionId) {
        filtered = filtered.filter((so) => so.sessionId === where.sessionId)
      }
      if (where?.locationId) {
        filtered = filtered.filter((so) => so.locationId === where.locationId)
      }
      if (where?.organizationId) {
        filtered = filtered.filter((so) => so.organizationId === where.organizationId)
      }

      return filtered.map((order) => {
        const result: any = { ...order }
        if (include?.lines) {
          result.lines = this.salesOrderLines.filter((line) => line.salesOrderId === order.id)
        }
        if (include?.customer) {
          result.customer = this.customers.find((c) => c.id === order.customerId) || null
        }
        if (include?.payments) {
          result.payments = this.payments.filter((p) => p.salesOrderId === order.id)
        }
        return result
      })
    },
    create: async ({ data }: { data: Omit<SalesOrder, "id" | "createdAt" | "updatedAt"> }) => {
