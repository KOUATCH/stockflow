...
export const posStationSchema = z.object({
  ...
  terminalNumber: z.string().min(1, "Terminal number is required").optional(),
})

export const createPosStationInputSchema = z.object({
  name: z.string().min(1, "Name is required").max(100, "Name must be less than 100 characters"),
  isActive: z.boolean().default(true),
  hasCashDrawer: z.boolean().default(true),
  locationId: z.string().min(1, "Location is required"),
...
export type CreatePosStationInput = z.infer<typeof createPosStationInputSchema>
