...
import { useLowStockMonitoring, useRealTimeBalanceTracking, useSystemMonitoring } from "@/hooks/cashDrawer/useRealTimeTracking"
...
export default function HomePage() {
  ...
  const { currentSession, sessionLoading } = useSessionManagementOld(selectedTerminal)
  ...
  useEffect(() => {
    if (isSessionActive && activeTab === "overview") {
      console.log("[v0] Session became active, switching to POS tab")
      setActiveTab("pos")
    }
  }, [isSessionActive, activeTab])

  useEffect(() => {
    if (!isSessionActive && (activeTab === "pos" || activeTab === "cash-drawer")) {
      console.log("[v0] Session became inactive, switching to overview tab")
            ...
            <div>
              ...
              <p className="text-gray-600 mt-2 flex items-center gap-3">
                ...
                <Badge variant="outline" className="flex items-center gap-1">
                  ...
                  {allTerminals.find((t) => t.id === selectedTerminal)?.name.split(" - ")[0] || "Terminal 1"}
                </Badge>
              </p>
            </div>
