import { getOrgPurchaseOrderById as getPurchaseOrder } from "@/actions/purchaseOrderWorkflow/purchaseOrderSystemAction"
          ...
          <InfoCard title="Order Details" icon={<DollarSign className="h-4 w-4" />}>
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold text-primary">{formatCurrency(displayGrandTotal)}</span>
              </div>
            </div>
          </InfoCard>
        </div>
              ...
              <table className="w-full">
                    ...
                    lines.map((line, index) => {
                      ...
                      return (
                        <tr key={line.id} className={index % 2 === 0 ? "bg-gray-50/50" : ""}>
                          ...
                          <td className="py-3 px-4 text-right font-medium">{qty}</td>
                          <td className="py-3 px-4 text-right">{formatCurrency(unitCost)}</td>
                          <td className="py-3 px-4 text-right font-medium">{formatCurrency(lineTotal)}</td>
                        </tr>
                      )
                    })
                  ) : (
                ...
                <tfoot>
                  <tr className="border-t-2 font-bold">
                    ...
                    <td className="py-3 px-4 text-right text-lg">{formatCurrency(displayGrandTotal)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            ...
            <div className="rounded-lg bg-muted/40 p-4 shadow-sm">
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-muted-foreground">{formatCurrency(displaySubtotal)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tax Amount:</span>
                  <span className="text-muted-foreground">{formatCurrency(displayTax)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Shipping:</span>
                  <span className="text-muted-foreground">{formatCurrency(displayShipping)}</span>
                </div>
                {displayDiscount > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Discount:</span>
                    <span className="text-destructive">-{formatCurrency(displayDiscount)}</span>
                  </div>
                )}
                <Separator />
                <div className="flex justify-between font-bold">
                  <span>Total Amount:</span>
                  <span className="text-primary">{formatCurrency(displayGrandTotal)}</span>
                </div>
              </div>
            </div>
          </div>
