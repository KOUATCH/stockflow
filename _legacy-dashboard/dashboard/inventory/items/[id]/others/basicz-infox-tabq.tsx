...
const productIdentitySchema = z.object({
  nameEn: z.string().min(1, 'English product name is required'),
...
function UpdateCard<T extends FieldValues>({
  ...
  const handleSubmit = useCallback(async (data: T) => {
    ...
    try {
      ...
      // You might want to show a provider notification here
    } finally {
...
export default function BasicInfoTab({ itemData, onUpdate }: BasicInfoTabProps) {
  ...
  const [orgItems, setOrgItems] = useState<ItemPayload[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (authenticatedUser?.organizationId) {
          ...
          const response = await getOrgItems(authenticatedUser.organizationId);
        ...
        {(form) => (
          <>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>English Product Name</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>French Product Name</FormLabel>
                  <FormControl>
                    <Input placeholder="Enter French product name" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
