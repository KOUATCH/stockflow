...
import { useSessionManagement } from "@/hooks/newPOSSession/useSessionManagement"; // Import useSessionManagement hook
...
export default function HomePage() {
  ...
  const handleLocationChange = (locationId: string) => {
    ...
    const locationTerminals = terminalData?.filter((terminal) => terminal.locationId === locationId) || []
    if (locationTerminals.length > 0) {
      setSelectedTerminalId(locationTerminals[0].id)
    }
  }
  ...
  // const { lowStockCount, criticalStockCount, lowStockItems } = useLowStockMonitoring(selectedLocation, organizationId)
  // const { systemMetrics, systemHealth } = useSystemMonitoring(organizationId, selectedLocation)
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <Select value={selectedTerminal} onValueChange={setSelectedTerminalId}>
      ...
      <div className="p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 text-balance">Welcome back, {user?.firstName}  {user?.lastName}</h2>
              <p className="text-gray-600 mt-2 flex items-center gap-3">
                <Badge variant="secondary" className="flex items-center gap-1">
                  <Users className="w-3 h-3" />
                  {user?.roles?.length ? user.roles.map((r) => r.name).join(", ") : "No Role"}
                ...
                <Badge variant="outline" className="flex items-center gap-1">
                  ...
                  {terminalData?.find((t) => t.id === selectedTerminalId)?.name.split(" - ")[0] || "Terminal 1"}
                </Badge>
              </p>
            </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {terminalData?.find((t) => t.id === selectedTerminalId)?.name || "Terminal 1"}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2">
                      <Power className={`w-4 h-4 ${isSessionActive ? "text-green-600" : "text-gray-400"}`} />
            ...
            <TabsContent value="reports" className="space-y-6">
              <Tabs defaultValue="analytics" className="space-y-4">
                ...
                <TabsContent value="analytics">
                  <AnalyticsDashboard locationId={selectedLocationId || ""} organizationId={orgId} />
                </TabsContent>
                <TabsContent value="reconciliation">
                  <CashReconciliationReportComponent locationId={selectedLocationId || ""} organizationId={orgId} />
                </TabsContent>
              </Tabs>
            </TabsContent>
            ...
            <TabsContent value="monitoring">
              <div className="space-y-6">
                <VarianceMonitor terminalId={selectedTerminalId} sessionId={currentSession?.id || ""} />
