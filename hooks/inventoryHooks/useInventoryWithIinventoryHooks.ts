...
export function useInfiniteItemsWithInventory(
  ...
  pageSize = 50,
  options?: Omit<UseInfiniteQueryOptions<FetchItemsResponse, Error>, 'queryKey' | 'queryFn' | 'getNextPageParam' | 'initialPageParam'>
) {
...
export function useSearchItemsWithInventory(
  ...
  return useQuery({
    ...
    queryFn: () => fetchItemsWithInventoryLevels({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return result.items.filter(item => {
        ...
        return inventory && 
               inventory.quantityOnHand <= Math.max(item.reorderLevel, inventory.reorderPoint)
      })
    },
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        totalValue: itemsWithInventory.reduce((sum, item) => 
          sum + (item.inventoryLevel?.totalValue || 0), 0
        ),
        ...
        outOfStockCount: itemsWithInventory.filter(item => 
          item.inventoryLevel!.quantityOnHand <= 0
        ).length,
        averageValue: itemsWithInventory.length > 0 
          ? itemsWithInventory.reduce((sum, item) => 
              sum + (item.inventoryLevel?.totalValue || 0), 0
