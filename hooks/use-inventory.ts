...
import { getInventoryLevels, getInventoryTransactions, getLowStockItems } from '@/actions/inventory/get-inventory-data';
...
export function useInventoryTransactions(itemId?: string, locationId?: string) {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const result = await getInventoryTransactions(itemId, locationId);
      if (result.success && result.data) {
