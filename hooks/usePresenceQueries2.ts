import { getPresenceAlerts, markAlertAsRead, resolveAlert } from "@/actions/presence/alertActions"
import {
  type ActivityType,
  clockInEmployee,
  clockOutEmployee,
...
import { generateAttendanceReport, getAttendanceAnalytics, getAttendanceReport } from "@/actions/presence/reportActions"
import { createEmployeeSchedule, getEmployeeSchedules } from "@/actions/presence/scheduleActions"
import { useNotifications } from "@/components/notifications/NotificationProvider"
import type { BreakType, ClockMethod } from "@/types/presence"
...
export const PresenceKeys = {
  ...
  userActivities: (userId: string, sessionId?: string) => [...PresenceKeys.activities(), userId, sessionId] as const,
}
...
export const useClockIn = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      success("Successfully Clocked In!", `You are now clocked in at ${new Date().toLocaleTimeString()}`, {
        category: "info",
        priority: "normal",
        action: {
          label: "View Status",
    ...
    onError: (err) => {
      error("Clock In Failed", err.message || "Unable to clock in at this time", {
        category: "error",
        priority: "high",
        action: {
          label: "Try Again",
...
export const useClockOut = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: PresenceKeys.all })
      success("Successfully Clocked Out!", `You are now clocked out at ${new Date().toLocaleTimeString()}`, {
        category: "info",
        priority: "normal",
        action: {
          label: "View Summary",
    ...
    onError: (err) => {
      error("Clock Out Failed", err.message || "Unable to clock out at this time", {
        category: "error",
        priority: "high",
        action: {
          label: "Try Again",
...
export const useStartBreak = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      success(`${breakTypeLabel} Started`, `You are now on ${breakTypeLabel.toLowerCase()}. Enjoy your time!`, {
        category: "info",
        priority: "low",
        action: {
    ...
    onError: (err) => {
      error("Failed to Start Break", err.message || "Unable to start break at this time", {
        category: "error",
        priority: "normal",
        action: {
          label: "Try Again",
...
export const useEndBreak = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: PresenceKeys.all })
      success("Break Ended", "Welcome back! You're now clocked in and ready to work", {
        category: "info",
        priority: "normal",
        action: {
          label: "View Status",
    ...
    onError: (err) => {
      error("Failed to End Break", err.message || "Unable to end break at this time", {
        category: "error",
        priority: "normal",
        action: {
          label: "Try Again",
...
export const useMarkAlertAsRead = () => {
  ...
  return useMutation({
    ...
    onError: (err) => {
      error("Failed to Mark Alert as Read", err.message || "Unable to update alert status", {
        category: "error",
        priority: "low",
      })
    },
...
export const useResolveAlert = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: PresenceKeys.alerts() })
      success("Alert Resolved", "The presence alert has been successfully resolved", {
        category: "info",
        priority: "normal",
    ...
    onError: (err) => {
      error("Failed to Resolve Alert", err.message || "Unable to resolve the alert at this time", {
        category: "error",
        priority: "normal",
        action: {
          label: "Try Again",
...
export const useGenerateAttendanceReport = () => {
  ...
  return useMutation({
    ...
    onError: (err) => {
      error("Report Generation Failed", err.message || "Unable to generate attendance report", {
        category: "error",
        priority: "normal",
        action: {
          label: "Try Again",
