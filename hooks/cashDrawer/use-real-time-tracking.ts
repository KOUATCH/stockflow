...
import { getCashDrawerSummary, getCurrentSession } from "@/actions/cashSystem/cash-drawer/cash-drawer-actions"
import { useQuery, useQueryClient } from "@tanstack/react-query"
import { useCallback, useEffect, useState } from "react"
import { useInventorySummary, useLowStockItems } from "../inventoryHooks/useInventoryWithIinventoryHooks"
// import { getCashDrawerSummary, getCurrentSession } from "@/actions/cash-drawer/cash-drawer-actions"
// import { useLowStockItems, useInventorySummary } from "@/hooks/use-items-with-inventory"
...
export function useRealTimeBalanceTracking(
  ...
  const realTimeState: RealTimeState = {
    ...
    sessionDuration: session ? (new Date().getTime() - new Date(session.startTime).getTime()) / (1000 * 60 * 60) : 0,
  }
...
export function useSessionTimeoutMonitoring(
  ...
  return {
    ...
    sessionDurationHours: session
      ? (new Date().getTime() - new Date(session.startTime).getTime()) / (1000 * 60 * 60)
      : 0,
    ...
    timeUntilTimeout: session
      ? Math.max(0, maxSessionHours - (new Date().getTime() - new Date(session.startTime).getTime()) / (1000 * 60 * 60))
      : 0,
  }
}
