...
const demoCredentials = ROLE_DEFINITIONS.map((role) => ({
  email: role.email,
...
async function seedAuthTables() {
  ...
  await prisma.account.createMany({
    data: seedIndexes.map((index) => ({
      ...
      type: "oauth",
      provider: "stockflow-seed",
      providerAccountId: `seed-provider-${pad(index)}`,
      access_token: `seed-access-token-${pad(index)}`,
      refresh_token: `seed-refresh-token-${pad(index)}`,
      token_type: "Bearer",
      scope: "profile email",
      expires_at: Math.floor(day(index + 30).getTime() / 1000),
      session_state: `seed-session-state-${pad(index)}`,
    })),
  });
  ...
  await prisma.session.createMany({
    data: seedIndexes.map((index) => ({
      id: id("session", index),
      sessionToken: `cmp-session-token-${pad(index)}`,
      userId: id("user", index),
      expires: day(index + 30),
    })),
  });
